...
checkoutBtn.addEventListener("click", async () => {
  ...
  // For now: choose supplier from first cart item
  const supplierId = cart[0].supplierId || "default";

  const orderData = {
    supplierId,
    items: cart,
    totalAmount: Number(cartTotalEl.textContent),
  };

  try {
    await api("/orders", "POST", orderData, true);
